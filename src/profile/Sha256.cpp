...
constexpr std::uint32_t rotate(std::uint32_t value, unsigned amount) noexcept {
...
std::string sha256Hex(std::string_view input) {
    ...
    for (std::size_t offset = 0; offset < bytes.size(); offset += 64U) {
        ...
        for (std::size_t i = 0; i < 16U; ++i) {
            ...
            words[i] = (static_cast<std::uint32_t>(bytes[j]) << 24U) |
...
bool isSha256Hex(std::string_view input) noexcept {
    if (input.size() != 64U)
        return false;
