...
ValidationResult makeCapturePlan(
    ...
    if (request.output_path.empty()) {
        addError(result.errors, "output_path is required");
    }
    if (!finitePositive(request.center_frequency_hz)) {
        addError(result.errors, "center_frequency_hz must be finite and positive");
    }
    if (!finitePositive(request.sample_rate_sps)) {
        addError(result.errors, "sample_rate_sps must be finite and positive");
    }
    if (request.bandwidth_hz.has_value() && !finitePositive(*request.bandwidth_hz)) {
        addError(result.errors, "bandwidth_hz must be finite and positive");
    }
    if (request.gain_db.has_value() && !std::isfinite(*request.gain_db)) {
        addError(result.errors, "gain_db must be finite");
    }
    if (request.duration_seconds.has_value() == request.sample_count.has_value()) {
        addError(result.errors, "exactly one of duration_seconds or sample_count is required");
    }
    if (request.duration_seconds.has_value() && !finitePositive(*request.duration_seconds)) {
        addError(result.errors, "duration_seconds must be finite and positive");
    }
    if (request.duration_seconds.has_value() &&
        *request.duration_seconds > limits.maximum_duration_seconds) {
        addError(result.errors, "duration_seconds exceeds the configured limit");
    }
    if (request.sample_count.has_value() && *request.sample_count == 0) {
        addError(result.errors, "sample_count must be positive");
    }
    if (request.read_timeout.count() <= 0 || request.read_timeout > limits.maximum_read_timeout) {
        addError(result.errors, "read_timeout is outside the configured bounds");
    }
    if (limits.maximum_consecutive_timeouts == 0 || limits.maximum_sample_count == 0 ||
        ...
        addError(result.errors, "resource limits are invalid");
    }
    ...
    result.plan = CapturePlan{request, effective, limits, target, bytes, rounding};
    return result;
}
