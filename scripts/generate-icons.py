...
MACOS_MODERN_SIZE = 1024
MACOS_MODERN_BACKGROUND_TOP = (74, 207, 252, 255)
MACOS_MODERN_BACKGROUND_BOTTOM = MACOS_MODERN_BACKGROUND_TOP
MACOS_MODERN_FACE_BOUNDS = (70, 70, 954, 954)
MACOS_MODERN_FACE_RADIUS = 145

GENERATED_ASSETS = (
    *(Path("assets/icons/linux") / f"sdr-calibration-{size}.png" for size in LINUX_SIZES),
    Path("assets/icons/macos/SDRCalibration.icns"),
    Path("assets/icons/macos/SDRCalibration-macOS26-1024.png"),
...
def write_modern_macos_source(full_master: Image.Image) -> None:
    """Create the opaque square artwork that macOS 26 masks at runtime."""
    size = MACOS_MODERN_SIZE
    background = Image.new("RGBA", (size, size))
    pixels = background.load()
    for y in range(size):
        mix = y / (size - 1)
        color = tuple(
            round(top * (1.0 - mix) + bottom * mix)
            for top, bottom in zip(
                MACOS_MODERN_BACKGROUND_TOP,
                MACOS_MODERN_BACKGROUND_BOTTOM,
                strict=True,
            )
        )
        for x in range(size):
    ...
    recovered = full_master.resize((size, size), Image.Resampling.LANCZOS)
    clear_detached_alpha(recovered)
    recovered_pixels = recovered.load()
    left, top, right, bottom = MACOS_MODERN_FACE_BOUNDS
    radius = MACOS_MODERN_FACE_RADIUS
    for y in range(size):
        for x in range(size):
            ...
            if inside_face:
                continue
            recovered_pixels[x, y] = (0, 0, 0, 0)

    composed = Image.alpha_composite(background, recovered).convert("RGB")
    encoded = io.BytesIO()
    composed.save(encoded, format="PNG", optimize=True)
    for output in (
        ROOT / "assets/icons/macos/SDRCalibration-macOS26-1024.png",
        ROOT
        / "assets/icons/macos/SDRCalibration.icon/Assets/"
        "SDRCalibration-macOS26-1024.png",
    ):
        output.parent.mkdir(parents=True, exist_ok=True)
...
def write_manifest() -> None:
    tracked_files = (
        ...
        Path("assets/icons/macos/SDRCalibration.icon/icon.json"),
        Path(
            "assets/icons/macos/SDRCalibration.icon/Assets/"
            "SDRCalibration-macOS26-1024.png"
        ),
    ...
    manifest = {
        ...
        "macos_modern": {
            "document": "assets/icons/macos/SDRCalibration.icon",
            "compiler": "Xcode actool",
            "minimum_deployment_target": "14.0",
            "pre_26_rendition": "generated by actool from the Icon Composer document",
        },
        "files": {
...
def main() -> None:
    ...
    write_modern_macos_source(full_master)
    write_manifest()
