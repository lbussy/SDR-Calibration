...
SDR Calibration produces evidence-bounded, per-device SDR frequency-calibration
profiles. A result is traceable only when its specific reference and provenance
evidence satisfy the documented claim contract. Its initial desktop targets are
...
development/windows-store-final-precertification-audit-execution-prompt
...
development/phase-14-5-branding-execution-prompt
...
development/reference-registry-contract
development/failure-state-model
development/testing
development/licensing-and-distribution
development/decisions/index
```
