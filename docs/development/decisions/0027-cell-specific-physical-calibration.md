...
satisfied. That attempt does not complete RC-01 or the release-wide RC-03 matrix.
...
- The current linear workflow needs at least one additional independently
  qualified Airspy observation at a different reference frequency. That
  frequency is a new RC-02 scope and requires its own immutable hardware plan.
...
- RC-03 remains `Not qualified` until a physical cell completes acquisition or
